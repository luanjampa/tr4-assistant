...
**Deployed to production**: Render (API) + Neon (Postgres+pgvector) + Cloudflare (embeddings/Turnstile), not Railway — Railway's hard spend-limit is workspace-wide only (confirmed against both official docs and the actual dashboard UI), not per-project, and would have taken down two unrelated existing projects on the same account if TR4 usage tripped it. Render free tier: 750h/month, 512MB RAM, sleeps after 15min idle (~30-50s cold start). Neon free tier: 100 compute-hours/month, 0.5GB storage against a current DB of ~23MB, no calendar-based expiry, resumes from idle in <500ms. Expected cost: **$0/month**, all three free tiers. Full steps in `docs/DEPLOY.md`, including a real production bug hit and fixed during the actual deploy: **Neon's pooled (PgBouncer) endpoint doesn't guarantee `public` on `search_path` for every connection handed out of the pool** — caused an intermittent `relation "tr4_usage" does not exist` even though the table existed; `store.py`'s pool `configure` callback (and `get_sync_conn`) now force `SET search_path TO public` on every connection (had to pair this with `conn.set_autocommit(True)` first — a bare `SET` without autocommit left pooled async connections in an `INTRANS` state that psycopg_pool's health check rejected). Migrating already-embedded data to a new Postgres instance (e.g. after this Cloudflare switch) doesn't need to re-run ingestion — `pg_dump --data-only --table=tr4_kb` from the source + `psql` into the target's connection string is much faster than re-running the ~40-60min Groq WhatsApp classification. Anthropic/Claude and self-hosted GPU were separately rejected earlier as too expensive/complex for chat generation specifically.
